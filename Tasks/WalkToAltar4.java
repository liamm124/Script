...
public abstract class WalkToAltar4 extends Task {


    public WalkToAltar4(ClientContext ctx) {
        super(ctx);
    }
    ...
    private Tile PreviousStep = new Tile(3027,5577,0);
    private Tile NextStep = new Tile(3045+Random.nextInt(-1,1),5577+Random.nextInt(-2,2),0);
    ...
    public boolean activate() {
        return ( ctx.players.local().tile().distanceTo(PreviousStep)<=7 && ctx.inventory.select().count()==28 )||(ctx.players.local().tile().distanceTo(PreviousStep)<=7 && ctx.inventory.select().count()==27);
    }
